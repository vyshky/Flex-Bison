...
%%
/*
нетерминал(Также нетерминал можно привязывать к value из union, чтобы понять как привызвать читай правила выше %type и %token): компонент_правила (действие)(пустая строка)( L + R )( $$ = 1$ + 10)( нетерминал + нетерминал )
	| компонент_правила
